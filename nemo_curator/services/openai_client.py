...
class OpenAIClient(LLMClient):
    ...
    def query_reward_model(self, *, messages: Iterable, model: str) -> dict:
        ...
        if not reward_content.startswith('reward'):
            raise ValueError(
                f"Invalid response format from reward model '{model}'. Expected 'reward: <score>', but got: {reward_content}"
            )
        try:
            reward_score = float(reward_content.split(':')[1])
        except (IndexError, ValueError) as e:
            raise ValueError(
                f"Failed to parse reward score from response content: {reward_content}. Error: {e}"
            )
        return reward_score
